...
In this example, it walks forward 100 pixels, turns 90° to the right, then walks
forward 50 pixels. The `done()` call at the end doesn't do anything here in the
browser, but it keeps the turtle window open, if you copy this script and run it
in regular Python. Can you change the script so that your canvas matches the
goal canvas below?
...
*Option+Up* and *Option+Down* on a Mac.
You can find more shortcuts in the [Ace editor documentation].
...
indentation to group lines into a loop, so the loop ends when the indentation
...
`up()` and `down()` calls around in this script to see what changes. Try adding
a `t.stamp()` call.
